...
1) Top 10% of codon adapted genes in each genome  
...
### Pull Top 10% of Codon Adapted Genes  
Take genome-wide CAI values, and for each species determine what the top 10% of codon adapted genes are. Will plot these relative to identified gene sets/families of interest.  

```{r pullTopTen}
dat.topTen <- dat.genomic.df %>%
  dplyr::filter(species != "Ce") %>%
  dplyr::select(geneID, Sr_CAI, species) %>%
  dplyr::mutate(Description = factor("Top10")) %>%
  dplyr::group_by(Description, species) %>%
  dplyr::slice_max(order_by = Sr_CAI, prop = .1) %>%
  dplyr::mutate(species = factor(species, levels = c("Ss", "Sr", "Sp", "Sv")))

```

### Plot Chemoreceptors, Top10, Astacins, and SCP/TAP CAI values  
...
tbl <- dat.SCPTAPS %>%
  ...
  rbind(.,dat.chemoR, dat.topTen, dat.astacin, dat.genome) %>%
  dplyr::mutate(Description = factor(Description, levels = c("Genome-wide", "Astacin", "SCP/TAP", "Chemoreceptor", "Top10")))
...
cai_plot <- ggplot(tbl, aes(Description,Sr_CAI, species)) +
  ...
  labs(title = "Species-specific codon adaptiveness",
       subtitle = "Genome-wide, SCP/TAP genes & Astacins (i.e. parasitism-associated genes), Chemoreceptors genes, Top 10% of codon adapted genes",
       x = "Gene Set Identity",
...
Gene groups plotted here include chemoreceptors, top 10% of codon adapted genes in genome, and members of the *Strongyloides* genome project families.  
``` {r}
cai_plot

```

## Functional Enrichment Analysis of Top10 Genes
Take lists of the top 10% of codon adapted genes in each species, and run either GO analysis or GSEA to see if these genes are enriched for anything interesting.

```{r functionalEnrichmentTop10, include=FALSE}


# Carry out GO enrichment of discarded gene set using gProfiler2 ----
top10.geneID <- dat.topTen %>%
  ungroup() %>%
  group_by(species) %>%
  dplyr::group_map(~ {
    gost.res <- .x %>%
...
names(top10.geneID) <- levels(dat.topTen$species)

gost.results <- lapply(names(top10.geneID), function(y){
  organism = case_when (y == "Ss" ~ "ststerprjeb528",
                        y == "Sr" ~ "strattprjeb125",
                        y == "Sp" ~ "stpapiprjeb525",
                        y == "Sv" ~ "stveneprjeb530")
  gost.res <- gost(top10.geneID[[y]]$geneID,
                   organism = organism, 
                   correction_method = "fdr")
})
names(gost.results) <- names(top10.geneID)

# Find GO terms that are enriched with p-values equal or small than a threshold.
enriched.terms <- lapply(names(top10.geneID), function(y){
  gost.results[[y]]$result %>%
    as_tibble() %>%
    dplyr::select(term_id, p_value)
}) 
names(enriched.terms) <- names(top10.geneID)

# Select highly enriched GO terms that are common to all species
highTerms.overlap <- enriched.terms %>%
  bind_rows(.id = "species") %>%
  dplyr::filter(p_value <= 1.0e-8) %>%
  group_by(term_id) %>%
  summarize(n= n()) %>%
  dplyr::filter(n >= 4)
...
I found GO terms that are commonly enriched in all *Strongyloides* species tested, with a p-value of at least 1e-8. This is a subset of 10 GO terms.  
```{r}
...
```{r saveFiles, include=FALSE, echo = TRUE}
...
# Save .csv lists of 10% of codon adapted genes per species
top10.geneID <- dat.topTen %>%
  ungroup() %>%
  group_by(species) %>%
  dplyr::group_map(~ {
    geneIDs <- .x %>%
      dplyr::select(geneID) %>%
      write.csv(paste0("./Data/Genomic/",.y$species,"_Top10_CAI.csv"))
  }, .keep = TRUE)


# Save plot of codon adaptivness in gene sets of interest
ggsave('./Plots/Top10_ChemoR_SGPF_CAI_Plot.pdf', plot = cai_plot, 
       width = 6, height = 4, 
       units = "in", device = "pdf", 
       useDingbats = FALSE)

# Save list of all significantly enriched GO terms for each species, 
# for the top 10% of codon adapted genes
lapply(names(top10.geneID), function(y){
  gost.results[[y]]$result %>%
    as_tibble() %>%
    ...
    write.csv(paste0("./Data/Genomic/",y,"_Top10_EnrichedTerms.csv"))
...
lapply(names(top10.geneID), function(y){
  ...
  gost.ggplot <- gostplot(gost.res, interactive = F, capped = T) +
    labs(title = paste0(y, "Top 10% Codon Adapted Genes")) +
    theme(plot.title.position = "plot",
  ...
  ggsave(paste0('./Plots/',y,'_Top10_CAI_Plot.pdf'), plot = gost.ggplot.pub,
         width = 4, height = 5,
         units = "in", device = "pdf",
         useDingbats = FALSE)
  
  publish_gosttable(gost.res,
                    highlight_terms = highTerms$overlap,
                    filename = paste0('./Plots/',y,'_Top10_CAI_Table.pdf'))

...
# S. papillosus ----
temp <- c(Sp = './Data/Chemoreceptors/Sp_chemoreceptors.csv')

genelist.Sp <- suppressWarnings(read.csv(temp, 
                                         header = TRUE, 
                                         colClasses = "character", 
                                         strip.white = T)) %>%
  as_tibble() 

## For papillosus, match wormbase gene sequence name to the 
## wbps geneID
##  and pull the cDNA sequence
Sp.tr.seq <- fetch_cDNAseq(species = 'stpapiprjeb525',
                           inputIDs = genelist.Sp$Gene.names,
                           inputFilter = 'wbps_gene_id')

write.table(Sp.tr.seq, 
            file = "./Data/Chemoreceptors/Sp_chemosensory_cDNA_list.csv", 
            sep = ",", 
            col.names = FALSE, 
            row.names = FALSE)
...
species.specific.GoI.dat.df$geneID <- str_remove_all(species.specific.GoI.dat.df$geneID, "\\.[0-9]$")
species.specific.GoI.dat.df$geneID <- str_remove_all(species.specific.GoI.dat.df$geneID, "[a-z]$")
```
...
ks.mean.pt.result.list <- lapply(GoI.dat.split.keys$species, function(x){
  ...
  display_dist_compare(dcomp.ks, 
                       pthresh, 
                       paste(x, 
              "- KS - GoI vs genomic (mean-centered)"))
})

# utility functions used to conduct permutation tests of mean value
#
...
# Get the difference in the means of the twotwo distributions
get_D_mean = function(curr_dist, ref_dist) {
    delta = mean(curr_dist) - mean(ref_dist)
}
...
#
permutation_run_mean = function(joint_dist, n_curr, n_ref) {
    n = n_curr + n_ref
    ix = sample.int(n, size=n, replace=FALSE)
    ix_curr = ix[1:n_curr]
    ix_ref = ix[(n_curr+1):n]
    
    get_D_mean(joint_dist[ix_curr], joint_dist[ix_ref])
...
#
boot_dist_compare_mean = function(currdist, refdist, nboots) {
    ncurr = length(currdist)
    nref = length(refdist)
    jointdist = c(currdist, refdist)
    
    D_mean_dist = vapply(1:nboots,
                         function(i) permutation_run_mean(jointdist, ncurr, nref),
                         numeric(1))
    
    D_mean_actual = get_D_mean(currdist, refdist)
    
    # cacluate proportion of sampled allocations that have an absoulte value of the 
    # mean diference at least as far from 0 as the sample data.
    p_est = sum(abs(D_mean_dist) >= abs(D_mean_actual))/nboots
    
    
    as_named_list(D_mean_actual, D_mean_dist, p_est)
...
#
display_dist_compare_mean = function(dcompare, pthresh = 0.002, title="") {
    unpack(dcompare, D_mean_actual, D_mean_dist, p_est)
    mesg = paste0("Delta mean = ", format(D_mean_actual, digits=3),
                  "\n estimated p = ", format(p_est, digits=3), "\n")
    
    if(p_est <= pthresh) {
        mesg = paste(mesg, "-- means appear different")
    } else {
        mesg = paste(mesg, "-- means do not appear different")
    }
    ggplot(data.frame(D=D_mean_dist), aes(x=D)) +
        geom_density(adjust=0.5) +
        geom_vline(xintercept = D_mean_actual,
                   color="maroon", linetype=2) +
        theme_bw() +
        labs(x = "Difference in mean CAI")+
